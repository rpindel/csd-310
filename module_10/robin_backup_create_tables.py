...
##
cursor.execute("CREATE TABLE wine_distributor_details (PK_Wine_ID INT(11) NOT NULL, PK_Distributor_ID INT(11) NOT NULL, PRIMARY KEY(Wine_ID), PRIMARY KEY(Distributor_ID));")

##
cursor.execute("CREATE TABLE distributor (Distributor_ID INT(11) NOT NULL, Name VARCHAR(45) NOT NULL, Street_Address_1 VARCHAR(35) NOT NULL, Street_Address_2 VARCHAR(35), Zip INT(5) NOT NULL, Contact_First_Name VARCHAR(25) NOT NULL, Contact_Last_Name VARCHAR(25) NOT NULL, Phone_Number VARCHAR(15) NOT NULL, Email_Address VARCHAR(45), Active BIT(1) NOT NULL, FK_Wine_ID INT(11) NOT NULL, FK_Wine_Order_ID INT(20) NOT NULL, PRIMARY KEY(Distributor_ID), CONSTRAINT fk_wine FOREIGN KEY(Wine_ID) REFERENCES wine(Wine_ID), CONSTRAINT fk_wine_order FOREIGN KEY(Wine_Order_ID) REFERENCES wine_order(Wine_Order_ID));")

##
cursor.execute("CREATE TABLE employee (Employee_ID INT(11) NOT NULL, First_Name VARCHAR(25) NOT NULL, Last_Name VARCHAR(25) NOT NULL, Hire_Date DATE NOT NULL, Start_Date DATE, Active BIT(1) NOT NULL, Department_ID INT(5) NOT NULL, Position_ID INT(10) NOT NULL, PRIMARY KEY(Employee_ID), CONSTRAINT fk_department FOREIGN KEY(Department_ID) REFERENCES department(Department_ID), CONSTRAINT fk_position FOREIGN KEY(Position_ID) REFERENCES position(Position_ID));")

##
cursor.execute("CREATE TABLE employee_alternate (Street_Address_1 VARCHAR(35) NOT NULL, Street_Address_2 VARCHAR(35), Zip INT(5) NOT NULL, Phone_Number VARCHAR(15) NOT NULL, Email_Address VARCHAR(45), Term_Date DATE, Term_Reason VARCHAR(25), Rehireable BIT(1), SSN INT(9) NOT NULL, DOB DATE, Employee_ID INT(11) NOT NULL, CONSTRAINT fk_employee FOREIGN KEY(Employee_ID) REFERENCES employee(Employee_ID));")

##
cursor.execute("CREATE TABLE employe_time_worked (Date DATE NOT NULL, Clock_In_Shift TIME NOT NULL, Clock_Out_Shift TIME NOT NULL, Clock_Out_Break TIME, Clock_In_Break TIME, Clock_Out_Lunch TIME, Clock_In_Lunch TIME, Employee_ID INT(11) NOT NULL, CONSTRAINT fk_employee FOREIGN KEY(Employee_ID) REFERENCES employee(Employee_ID));")

##
cursor.execute("CREATE TABLE zip_lookup (Zip INT(5) NOT NULL, City VARCHAR(25) NOT NULL, State VARCHAR(10) NOT NULL, Country VARCHAR(20), PRIMARY KEY(Zip));")

##
cursor.execute("CREATE TABLE department (Department_ID INT(5) NOT NULL, Department_Name VARCHAR(15) NOT NULL, Department_Head INT(11), PRIMARY KEY(Department_ID));")

##
cursor.execute("CREATE TABLE position (PK_Position_ID INT(10) NOT NULL, Position_Title VARCHAR(25) NOT NULL, Pay_Grade INT(11), Hourly BIT(1), Supervisory BIT(1), PRIMARY KEY(Position_ID));")
